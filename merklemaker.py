...
class merkleMaker(threading.Thread):
	...
	def makeClear(self):
		...
		self._makeOne(self.clearMerkleRoots.put, self.clearMerkleTree, height=self.currentBlock[1], False)
	
	def makeNext(self):
		self._doing('longpoll merkle roots')
		self._makeOne(self.nextMerkleRoots.put, self.clearMerkleTree, height=self.currentBlock[1] + 1, False)
